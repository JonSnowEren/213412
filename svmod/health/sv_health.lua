...
-- Arabadan çıkarken hasar verme hook'u güncellendi
hook.Add("SV_PlayerLeaveVehicle", "SV_DealDamageOnLeave", function(ply, veh)
	...
	-- Oyuncunun mevcut can ve zırhını kaydet
	...
	-- Hasar verme işlemini bir frame sonraya ertele
	timer.Simple(0.1, function()
		if not IsValid(ply) then return end
		
		-- Eğer oyuncu bayılmışsa (ragdoll olduysa)
		if ply:GetNWBool("IsRagdolled", false) or ply.KnockedOut or ply.isArrested then
			-- Can ve zırhı geri yükle
			timer.Simple(0.5, function()
				if IsValid(ply) and ply:Alive() then
					ply:SetHealth(math.max(1, currentHealth - DInfo:GetDamage()))
...
-- Arabadan çıkış zamanını işaretle
hook.Add("PlayerLeaveVehicle", "SV_MarkVehicleExit", function(ply, veh)
	if SVMOD:IsVehicle(veh) then
		ply.SV_LastVehicleExit = CurTime()
		ply.SV_SavedHealth = ply:Health()
		ply.SV_SavedArmor = ply:Armor()
	end
end)

-- Ghost efektini önlemek için
hook.Add("PlayerSpawn", "SV_PreventGhostAfterCrash", function(ply)
	-- Eğer oyuncu arabadan çıktıktan sonra spawn oluyorsa
	if ply.SV_LastVehicleExit and (CurTime() - ply.SV_LastVehicleExit) < 5 then
		-- Ghost efektini kaldır
		timer.Simple(0.1, function()
			if IsValid(ply) and ply:Alive() then
				ply:SetNoDraw(false)
				...
				-- Kaydedilen can ve zırhı geri yükle
				if ply.SV_SavedHealth then
					ply:SetHealth(ply.SV_SavedHealth)
					ply.SV_SavedHealth = nil
				end
				if ply.SV_SavedArmor then
					ply:SetArmor(ply.SV_SavedArmor)
					ply.SV_SavedArmor = nil
				end
			end
		end)
		
		ply.SV_LastVehicleExit = nil
	end
end)

-- DarkRP uyumluluğu için
hook.Add("onPlayerUnconscious", "SV_SaveHealthOnKnockout", function(ply, inflictor)
	if IsValid(inflictor) and SVMOD:IsVehicle(inflictor) then
		-- Bayılmadan önceki can ve zırhı kaydet
		ply.SV_PreKnockoutHealth = ply:Health()
		ply.SV_PreKnockoutArmor = ply:Armor()
	end
end)

hook.Add("onPlayerWakeUp", "SV_RestoreHealthAfterKnockout", function(ply)
	if ply.SV_PreKnockoutHealth then
		timer.Simple(0.1, function()
			if IsValid(ply) and ply:Alive() then
				ply:SetHealth(ply.SV_PreKnockoutHealth)
				ply:SetArmor(ply.SV_PreKnockoutArmor or 0)
				
				-- Ghost efektini temizle
				ply:SetNoDraw(false)
				ply:SetNotSolid(false)
				ply:SetMoveType(MOVETYPE_WALK)
				ply:SetCollisionGroup(COLLISION_GROUP_PLAYER)
		...
		ply.SV_PreKnockoutHealth = nil
		ply.SV_PreKnockoutArmor = nil
	end
end)