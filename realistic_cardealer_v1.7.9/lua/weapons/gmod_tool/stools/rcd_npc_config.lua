	...
	if SERVER then
		...
		if not placePlateform then
			...
			if IsValid(ent) && ent:GetClass() == "rcd_cardealer" then
				...
				RCD.CreateNPC(nil, "Default Name", "models/breen.mdl", pos, angSet, {}, {}, string.lower(game.GetMap()), false, false)
			end
			...
			net.Start("RCD:Admin:Configuration")
				...
				net.WriteBool(npcInfo["disableShop"])
				net.WriteBool(npcInfo["disableGarage"])
...
function TOOL:RightClick(trace)
	if SERVER then
		...
		if ent:GetClass() != "rcd_cardealer" then return end
