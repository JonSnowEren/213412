...
function RCD.loadMenuInformation(ent, vehicleId, class, price)
	...
	layoutMenu.Paint = function(self, w, h, x, y)
		...
		draw.DrawText(RCD.GetSetting("serverName", "string"), "RCD3D:Font:01", w/2, h/2-20, color_black, TEXT_ALIGN_CENTER)
	...
	if isstring(class) then
		...
		ent.vehicleInfo = {
			...
			["price"] = price,
			["vehicleId"] = vehicleId,
			["commission"] = vehicleTable["options"]["minCommissionPrice"],
		}
...
function RCD.OpenShowcaseMenu(ent, vehicleId, class)
	...
	for k, v in pairs(RCD.AdvancedConfiguration["vehiclesList"]) do
		...
		modelCars2.DoClick = function()
			local vehicleTable = RCD.AdvancedConfiguration["vehiclesList"][k] or {}
			vehicleTable["options"] = vehicleTable["options"] or {}
	...
	if ent.layoutCarOpen && layoutCar:GetOpen() then
		layoutCar:SetOpen(false)
	...
	local commission = ent.vehicleInfo["commission"] or min
	...
	sliderB.x = (3325-1550)*((commission-min)/(max-min))+1550
	sliderB.Paint = function(self,w,h,x,y)
		...
		if self.StartGrag then
			...
			commission = math.Round(ratio, 1)*(max-min)+min
		end
